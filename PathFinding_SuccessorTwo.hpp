...
public:
	...
	bool VerifyPossbilePatchs_SuccessorTwo(std::array<std::array<char, 10>, 10>& BotArray, int startingRow, int startingColumn, int shipSize,int previousHorizontalVector,int previousVerticalVector)
	{
		...
		chooseRandomAgain:
			...
			shipSize--;
			_successorThreeHavePath = VerifyPossbilePatchs_SuccessorThree(BotArray, _updatedRowPosition, _updatedColumnPosition, shipSize, randomHorizontalVector, randomVerticalVector);
			if (_successorThreeHavePath && _successorTwoHavePath)
			{
				return true;
			}
			return false;
